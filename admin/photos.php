...
$txtcolor1c = "#000";
$txtcolor2c = "#003399";
$filefolder = "../img/persons/";
$sitetitle = 'Visuels pour le tableau des affaires des Jours';
$user = 'admin';
$pass = 'admin';
...
if ($folder == '') {
  ...
  if (!preg_match('/'.preg_quote($filefolder, '/').'/',$folder)) {
    $folder = $filefolder;
  }
}
...
/*      ."<tr><td align=\"left\"><font face=\"Arial\" color=\"black\" size=\"4\">$sitetitle</font><font size=\"3\" color=\"black\"> :: $title</font></td>\n"
  ...
  if ($showtop) {
    ...
    $urlGsheet = 'https://docs.google.com/spreadsheets/d/1n3p4pY6FFmQGfbYn1NBpmOt3KFaPyhqMDh4YAVKGfro/edit#gid=1850485848';
    $urlDataviz = 'http://apps.dansmonlabo.com/lj-tableau-affaires/'; 

    echo "
    <tr>
    <td>
    <div id=\"header\">
      <h1>Gestion des images du tableau des affaires des Jours</h1>
      <p>Pour chaque personnalité, reporter le nom du fichier image dans la colonne 'img_fichier' de la Google Sheet</p>
    </div>  
    </td>
    </tr>
    <tr><td><font size=\"2\">\n"
        ."<a href=\"".$adminfile."?op=home\" $iftop>Accueil</a>\n"
        ."::\n<a href=\"".$adminfile."?op=up\" $iftop>Télécharger un fichier</a>\n"
        ."::\n<a href=\"".$urlGsheet."\" target=\"_blank\" $iftop>Gérer les textes</a>\n"
        ."::\n<a href=\"".$urlDataviz."\" target=\"_blank\" $iftop>Afficher le minisite</a>\n";
...
function login($er=false) {
  global $op;
    ...
    if ($er) { 
		echo "<font class=error>**Erreur : mauvais nom d’utilisateur ou mot de passe.**</font><br><br>\n"; 
	}
...
function home() {
  ...
  if ($folder) {
    if (preg_match('/\/home\//',$folder)) {
      $folderx = preg_replace('/'.preg_quote($filefolder, '/').'/', "", $folder);
    ...
    if ($stylesheet[0] != "." && $stylesheet[0] != ".." ) {
      if (is_dir($folder.$stylesheet) && is_readable($folder.$stylesheet)) { 
        $content1[$a] ="<td>".$sstylesheet."</td>\n"
                 ...
                 ."<td align=\"center\"><a href=\"".$adminfile."?op=ren&file=".$stylesheet."&folder=$folder\">Renommer</a>\n"
                 ."<td align=\"center\"><a href=\"".$adminfile."?op=del&dename=".$stylesheet."&folder=$folder\">Supprimer</a>\n"
        ...
        $content2[$b] ="
                 <td align=\"left\"><a href=\"".$adminfile."?op=viewframe&file=".$stylesheet."&folder=$folder\"><img src=\"".$folderx.$stylesheet."\" width=\"100\"></a>\n"
                 ."<td align=\"left\" width=\"300\">".$stylesheet."\n"
                 /*."<td align=\"left\">".filesize($folder.$stylesheet).*/
                 /*."<td align=\"center\"><a href=\"".$adminfile."?op=edit&fename=".$stylesheet."&folder=$folder\">Edit</a>\n"*/
                 ."<td align=\"center\"><a href=\"".$adminfile."?op=ren&file=".$stylesheet."&folder=$folder\">Renommer</a>\n"
                 ."<td align=\"center\"><a href=\"".$adminfile."?op=del&dename=".$stylesheet."&folder=$folder\">Supprimer</a>\n"
                 /*."<td align=\"center\"><a href=\"".$adminfile."?op=mov&file=".$stylesheet."&folder=$folder\">Move</a>\n"*/
                 /*."<td align=\"center\"><a href=\"".$adminfile."?op=viewframe&file=".$stylesheet."&folder=$folder\">Afficher</a>\n"*/
                 ."<tr height=\"2\"><td height=\"2\" colspan=\"3\">\n";
        $b++;
      } else {
        echo "Impossible de lire le contenu de ce dossier\n";
      }
    $count++;
    } 
  }
...
function up() {
  ...
  echo "<FORM ENCTYPE=\"multipart/form-data\" ACTION=\"".$adminfile."?op=upload\" METHOD=\"POST\">\n"
      ."<font face=\"tahoma\" size=\"2\"><b>Fichier\n:</b></font><br><input type=\"File\" name=\"upfile\" size=\"20\" class=\"text\">\n"

      ."<br><br>Emplacement\n:<br><select name=\"ndir\" size=1>\n"
...
function upload($upfile, $ndir) {
  ...
  if (!$upfile) {
    ...
    if(copy($upfile['tmp_name'],$ndir.$upfile['name'])) { 
      maintop("Upload");
      echo "Le fichier ".$upfile['name'].$folder.$upfile_name." a été correctement téléchargé.\n";
      mainbottom();
    } else {
      printerror("Le fichier $upfile n'a pas été téléchargé.");
    }
  } else {
    printerror("Merci de sélectionner un fichier.");
  }
}
...
function del($dename) {
  global $folder;
    ...
    echo "<table border=\"0\" cellpadding=\"2\" cellspacing=\"0\">\n"
        ."<font class=error>Attention, vous êtes sur le point de supprimer ".$folder.$dename.". <br> Cette action ne peut être annulée.</font><br><br>\n"
        ."Etes-vous sûr&nbsp;?<br><br>\n"
        ."<a href=\"".$adminfile."?op=delete&dename=".$dename."&folder=$folder\">Oui, supprimer</a> | \n"
        ."<a href=\"".$adminfile."?op=home\">Non, annuler </a>\n"
        ."</table>\n";
    mainbottom();
  } else {
    home();
  ...
  if (!$dename == "") {
    ...
    if (is_dir($folder.$dename)) {
      if(rmdir($folder.$dename)) {
        echo $dename." a été supprimé.";
      } else {
        echo "Il y a eu un problème lors de la suppression de ce dossier.";
      }
    } else {
      if(unlink($folder.$dename)) {
        echo $dename." a été supprimé.";
      } else {
        echo "Il y a eu un problème lors de la suppression de ce fichier.";
      }
    }
    mainbottom();
  } else {
...
function edit($fename) {
  ...
  if (!$fename == "") {
   ...
   $contents = "";
    ...
    $contents =  preg_replace('/'.preg_quote($replacea, '/').'/', $replaceb, $contents);
...
function save($ncontent, $fename) {
  ...
  if (!$fename == "") {
    ...
    $replacea = $replace1.$replace2;
    $replaceb = $replace3.$replace4;
    $ncontent = preg_replace('/'.preg_quote($replaceb, '/').'/', $replacea, $ncontent);
...
function ren($file) {
  ...
  if (!$file == "") {
    ...
    echo "<form action=\"".$adminfile."?op=rename\" method=\"post\">\n"
        ...
        ."Renommer ".$folder.$file;

    echo "</table><br>\n"
        ."<input type=\"hidden\" name=\"rename\" value=\"".$file."\">\n"
        ."<input type=\"hidden\" name=\"folder\" value=\"".$folder."\">\n"
        ."Nouveau nom :<br><input class=\"text\" type=\"text\" size=\"20\" name=\"nrename\">\n"
...
function renam($rename, $nrename, $folder) {
  ...
  if (!$rename == "") {
    ...
    if(rename($loc1,$loc2)) {
      echo "Le fichier ".$folder.$rename." a été renommé en <a href=\"".$adminfile."?op=viewframe&file=".$nrename."&folder=$folder\">".$folder.$nrename."</a>\n";
    } else {
      echo "Oups ! Impossible de renommer ce fichier\n";
    }
    mainbottom();
  } else {
    home();
